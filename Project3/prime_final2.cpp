...
void *erase_multiple_long(void *arg) {
    // long_list의 start_position부터 시작하여
    // jumpsize만큼 건너뛰며 해당 위치 원소를 1 -> 0으로 변경
    // num_jump회 지우면 종료 or N을 넘어가면 종료
    // 참고 : jumpsize는 이미 소수임이 공인됨 (short_list 때 확인)
    MultipleArg *multiple_arg = (MultipleArg *) arg;
    LL n = multiple_arg->start_position;
    LL nj = multiple_arg->num_jump;
    int js = multiple_arg->jumpsize;
    for (LL j = 0; j < nj; ++j) {
        multiple_arg->ll->is_prime[n] = 0;
        n += js;
        if (n >= multiple_arg->ll->size) {
            break;
        }
    }
    return NULL;
...
void *erase_long(void *arg) {
    ...
    // jumpsize만큼 건너뛰며 그의 배수인 합성수를 지우는 역할
    // 하나의 jumpsize에 대해, 전체를 여러 개로 분할시켜 multi-threads 작업
    int n = sqrt(ll.size);
    int jumpsize;
    while (get_long_jumper(&lj) != 0) {
        jumpsize = inc_long_jumper(&lj);
        pthread_t p_threads[M];
        LL start_position = 2 * jumpsize;
        LL num_jump = N / jumpsize;
        MultipleArg **multiple_args = new MultipleArg*[M];
        for (int i=0; i<M; ++i) {
            multiple_args[i]->ll = &ll;
            multiple_args[i]->start_position = start_position;
            multiple_args[i]->num_jump = num_jump / M;
            multiple_args[i]->jumpsize = jumpsize;
            cout << "Hello" <<endl;
            pthread_create(&p_threads[i], NULL, erase_multiple_long, (void *) multiple_args[i]);
            start_position += jumpsize; 
            if (start_position > N) {
                break;
            }
        }
        start_position = 2 * jumpsize;
        for (int i=0; i<M; ++i) {
        ...
        for (int i=0; i<M; ++i) {
            delete[] multiple_args[i];
        }
        delete[] multiple_args;
    }
    return NULL;
}
